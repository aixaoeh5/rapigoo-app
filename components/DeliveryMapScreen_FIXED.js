...
const DeliveryMapScreen = () => {
    ...
    // FIX: Mejorar loadActiveDeliveries con proper cleanup
    const loadActiveDeliveries = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                // FIX: Solo actualizar estado si el componente está montado
                ...
                if (deliveries.length > 0) {
                    ...
                    if (navigationStates.includes(activeDelivery.status)) {
                        ...
                        // FIX: Limpiar timeout anterior y usar ref para cleanup
                        if (navigationTimeoutRef.current) {
                            clearTimeout(navigationTimeoutRef.current);
                        }
                        
                        navigationTimeoutRef.current = setTimeout(() => {
                            // FIX: Verificar que el componente sigue montado antes de navegar
    ...
    const loadAvailableOrders = async () => {
        try {
            ...
            // FIX: Solo actualizar estado si está montado
            if (isMountedRef.current) {
                setAvailableOrders([]);
            }
        }
    ...
    // FIX: Mejorar getLocationPermission con mejor error handling
...
const styles = StyleSheet.create({
    ...
    // FIX: Nuevo estilo para loading container
    loadingContainer: {
        flex: 1,
